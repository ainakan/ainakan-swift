...
public final class PortalService: @unchecked Sendable, CustomStringConvertible, Equatable, Hashable {
    ...
    public init(clusterParameters: EndpointParameters, controlParameters: EndpointParameters? = nil) {
        handle = ainakan_portal_service_new(clusterParameters.handle, controlParameters?.handle)
    ...
    public func start() async throws {
        try await ainakanAsync(Void.self) { op in
            ainakan_portal_service_start(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_portal_service_start_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func stop() async throws {
        try await ainakanAsync(Void.self) { op in
            ainakan_portal_service_stop(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_portal_service_stop_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func kick(_ connectionId: ConnectionID) {
        ainakan_portal_service_kick(handle, connectionId)
    }
    ...
    public func post(
        ...
        ainakan_portal_service_post(handle, connectionId, json, rawData)
    ...
    public func narrowcast(
        ...
        ainakan_portal_service_narrowcast(handle, tag, json, rawData)
    ...
    public func broadcast(
        ...
        ainakan_portal_service_broadcast(handle, json, rawData)
        g_bytes_unref(rawData)
    }

    public func enumerateTags(for connectionId: ConnectionID) -> [String] {
        var length: gint = 0
        guard let rawVector = ainakan_portal_service_enumerate_tags(handle, connectionId, &length) else {
            return []
        }
    ...
    public func tag(_ connectionId: ConnectionID, tag: String) {
        ainakan_portal_service_tag(handle, connectionId, tag)
    }

    public func untag(_ connectionId: ConnectionID, tag: String) {
        ainakan_portal_service_untag(handle, connectionId, tag)
    }
