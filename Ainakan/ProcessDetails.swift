...
public final class ProcessDetails: CustomStringConvertible, Equatable, Hashable, Identifiable {
    ...
    public var pid: UInt {
        return UInt(ainakan_process_get_pid(handle))
    }

    public var name: String {
        return String(cString: ainakan_process_get_name(handle))
    }

    public lazy var parameters: [String: Any] = {
        return Marshal.dictionaryFromParametersDict(ainakan_process_get_parameters(handle))
    }()
    ...
    public var description: String {
        return "Ainakan.ProcessDetails(pid: \(pid), name: \"\(name)\", parameters: \(parameters))"
    }
