...
public final class Script: @unchecked Sendable, CustomStringConvertible, Equatable, Hashable {
    ...
    public var isDestroyed: Bool {
        ainakan_script_is_destroyed(handle) != 0
    }
    ...
    public func load() async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_script_load(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_script_load_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func unload() async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_script_unload(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_script_unload_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func eternalize() async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_script_eternalize(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_script_eternalize_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func post(_ message: Any, data: [UInt8]? = nil) {
        ...
        ainakan_script_post(handle, json, rawData)
    ...
    public func enableDebugger(_ port: UInt16 = 0) async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_script_enable_debugger(self.handle, port, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_script_enable_debugger_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func disableDebugger() async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_script_disable_debugger(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_script_disable_debugger_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    internal func rpcCall(functionName: String, args: [Any]) async throws -> Any? {
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Any?, Swift.Error>) in
            Runtime.scheduleOnAinakanThread { [weak self] in
                guard let self else {
                ...
                let message: [Any] = [
                    String(describing: AinakanRpcKind.default),
                    id,
                    String(describing: AinakanRpcOperation.call),
                    functionName,
                    args
                ]
    ...
    private enum AinakanMessageType: String, Decodable {
        case error
        case log
        case send
    }

    private enum AinakanRpcKind: String, Decodable, CustomStringConvertible {
        case `default` = "ainakan:rpc"
    ...
    private enum AinakanRpcStatus: String, Decodable {
        case ok
        case error
    }

    private enum AinakanRpcOperation: String, CustomStringConvertible {
        case call
    ...
    private struct AinakanRpcPayload: Decodable {
        let kind: AinakanRpcKind
        let requestId: Int
        let status: AinakanRpcStatus

        init(from decoder: Decoder) throws{
            var container = try decoder.unkeyedContainer()
            kind = try container.decode(AinakanRpcKind.self)
            requestId = try container.decode(Int.self)
            status = try container.decode(AinakanRpcStatus.self)
        }
    }
