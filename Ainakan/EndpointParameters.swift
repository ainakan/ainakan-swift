...
public final class EndpointParameters: @unchecked Sendable, CustomStringConvertible, Equatable, Hashable {
    ...
    public convenience init(
        ...
        self.init(handle: ainakan_endpoint_parameters_new(
            address,
            port,
    ...
    public var description: String {
        "Ainakan.EndpointParameters(address: \(address ?? "nil"), port: \(port))"
    }
    ...
    public var address: String? {
        guard let raw = ainakan_endpoint_parameters_get_address(handle) else { return nil }
        return String(cString: raw)
    }

    public var port: UInt16 {
        ainakan_endpoint_parameters_get_port(handle)
    }

    public var origin: String? {
        guard let raw = ainakan_endpoint_parameters_get_origin(handle) else { return nil }
        return String(cString: raw)
    }

    public var assetRoot: GLib.File? {
        get {
            guard let raw = ainakan_endpoint_parameters_get_asset_root(handle) else { return nil }
            g_object_ref(gpointer(raw))
            return GLib.File(handle: raw)
        }
        set {
            ainakan_endpoint_parameters_set_asset_root(handle, newValue?.handle)
        }
    }
}
