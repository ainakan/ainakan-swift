...
class Marshal {
    ...
    static func takeNativeError(_ error: UnsafeMutablePointer<GError>) -> Swift.Error {
        ...
        if domain == ainakan_error_quark() {
            let ainakanCode = AinakanError(UInt32(code))

            switch ainakanCode {
            case AINAKAN_ERROR_SERVER_NOT_RUNNING:
                return Error.serverNotRunning(message)
            case AINAKAN_ERROR_EXECUTABLE_NOT_FOUND:
                return Error.executableNotFound(message)
            case AINAKAN_ERROR_EXECUTABLE_NOT_SUPPORTED:
                return Error.executableNotSupported(message)
            case AINAKAN_ERROR_PROCESS_NOT_FOUND:
                return Error.processNotFound(message)
            case AINAKAN_ERROR_PROCESS_NOT_RESPONDING:
                return Error.processNotResponding(message)
            case AINAKAN_ERROR_INVALID_ARGUMENT:
                return Error.invalidArgument(message)
            case AINAKAN_ERROR_INVALID_OPERATION:
                return Error.invalidOperation(message)
            case AINAKAN_ERROR_PERMISSION_DENIED:
                return Error.permissionDenied(message)
            case AINAKAN_ERROR_ADDRESS_IN_USE:
                return Error.addressInUse(message)
            case AINAKAN_ERROR_TIMED_OUT:
                return Error.timedOut(message)
            case AINAKAN_ERROR_NOT_SUPPORTED:
                return Error.notSupported(message)
            case AINAKAN_ERROR_PROTOCOL:
                return Error.protocolViolation(message)
            case AINAKAN_ERROR_TRANSPORT:
                return Error.transport(message)
            default:
                fatalError("Unexpected Ainakan error code \(code)")
            }
        }
    ...
    static func iconFromVarDict(_ dict: [String: Any]) -> Icon {
        ...
        default:
            fatalError("Unexpected icon format from Ainakan: \(formatString)")
        }
    }
