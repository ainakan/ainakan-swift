...
public final class DeviceManager: @unchecked Sendable {
    ...
    public init() {
        ...
        handle = ainakan_device_manager_new()
    ...
    public func close() async throws {
        try await ainakanAsync(Void.self) { op in
            ainakan_device_manager_close(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                    let op = InternalOp<Void>.takeRetained(from: userData!)

                    var rawError: UnsafeMutablePointer<GError>? = nil
                    ainakan_device_manager_close_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func addRemoteDevice(
        ...
        let options = ainakan_remote_device_options_new()
        defer { g_object_unref(gpointer(options)) }

        if let certificate {
            let rawCertificate = try Marshal.certificateFromString(certificate)
            ainakan_remote_device_options_set_certificate(options, rawCertificate)
        ...
        if let keepaliveInterval {
            ainakan_remote_device_options_set_keepalive_interval(options, gint(keepaliveInterval))
        }

        g_object_ref(gpointer(options))

        let handle = try await ainakanAsync(OpaquePointer.self) { op in
            ainakan_device_manager_add_remote_device(self.handle, address, options, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                    let op = InternalOp<OpaquePointer>.takeRetained(from: userData!)

                    var rawError: UnsafeMutablePointer<GError>? = nil
                    let rawDeviceHandle = ainakan_device_manager_add_remote_device_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    public func removeRemoteDevice(address: String) async throws {
        try await ainakanAsync(Void.self) { op in
            ainakan_device_manager_remove_remote_device(self.handle, address, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                    let op = InternalOp<Void>.takeRetained(from: userData!)

                    var rawError: UnsafeMutablePointer<GError>? = nil
                    ainakan_device_manager_remove_remote_device_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
    ...
    private func fetchInitialDevices() async throws -> [Device] {
        try await ainakanAsync([Device].self) { op in
            ainakan_device_manager_enumerate_devices(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                    let op = InternalOp<[Device]>.takeRetained(from: userData!)

                    var rawError: UnsafeMutablePointer<GError>? = nil
                    let rawDevices = ainakan_device_manager_enumerate_devices_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
                    ...
                    let count = ainakan_device_list_size(rawDevices)
                    for i in 0 ..< count {
                        let devHandle = ainakan_device_list_get(rawDevices, i)!
