...
public final class PortalMembership: @unchecked Sendable, CustomStringConvertible, Equatable, Hashable {
    ...
    public var description: String {
        return "Ainakan.PortalMembership()"
    }
    ...
    public func terminate() async throws {
        return try await ainakanAsync(Void.self) { op in
            ainakan_portal_membership_terminate(self.handle, op.cancellable, { sourcePtr, asyncResultPtr, userData in
                let op = InternalOp<Void>.takeRetained(from: userData!)

                var rawError: UnsafeMutablePointer<GError>? = nil
                ainakan_portal_membership_terminate_finish(OpaquePointer(sourcePtr), asyncResultPtr, &rawError)
